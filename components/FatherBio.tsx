...
interface FatherBioProps {
  father: Father;
  onBack: () => void;
}

const FatherBio: React.FC<FatherBioProps> = ({ father, onBack }) => {
  const navigate = useNavigate();
  const detailedBio = father.bio || `With deep spiritual wisdom and decades of service, ${father.name.split(' ')[0]} brings transformative guidance to The Sonship Hub. Their commitment to excellence and faith-driven leadership has shaped countless lives.`;

  const handleBack = () => {
    navigate('/');
    setTimeout(() => {
      const section = document.getElementById('fathers');
      if (section) {
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-start">
          ...
          <div className="lg:col-span-5 relative">
            ...
            <div className="mt-8 flex items-center justify-between p-6 bg-brand-purple text-white rounded-sm shadow-xl">
              <div className="flex flex-col">
                ...
                <span className="text-xs font-medium opacity-60">Connect with {father.name.split(' ')[0]}</span>
              </div>
          ...
          <div className="lg:col-span-7">
            <div className="mb-10">
              <h4 className="text-brand-purple font-black tracking-[0.3em] text-[10px] uppercase mb-3">TSH Leadership</h4>
              ...
              <div className="flex items-center space-x-4">
                ...
                <span className="text-brand-purple font-black text-sm uppercase tracking-widest italic">{father.role}</span>
              </div>
            </div>
